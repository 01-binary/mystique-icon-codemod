// icon-transform.js
import { ASTPath, ImportDeclaration, JSXAttribute, JSXElement, JSXIdentifier, JSXOpeningElement } from 'jscodeshift';
...
function getNewIconComponentName(iconString) {
  ...
  } else if (namePart.startsWith('ic_')) { // 'ic_basic_' 외의 'ic_' 접두사도 처리
...
export default function transformer(file, api) {
  ...
  root.findJSXElements(oldIconDefaultImportName).forEach(path => {
    ...
    if (iconPropValue) {
      ...
      if (!colorPropExists) {
        newAttributes.push(
          j.jsxAttribute(j.jsxIdentifier('color'), j.stringLiteral('currentColor'))
        );
      }
  ...
  root
    ...
    .forEach(path => {
        const remainingSpecifiers = path.node.specifiers.filter(s => {
            return !(s.type === 'ImportDefaultSpecifier' && s.local.name === oldIconDefaultImportName);
        });
